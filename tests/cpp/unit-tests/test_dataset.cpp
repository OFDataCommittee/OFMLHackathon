...
SCENARIO("Testig DataSet object", "[DataSet]")
{
    GIVEN("A DataSet object")
    {
        ...
        WHEN("A tensor is added to the DataSet")
        {
            ...
            THEN("The tensor can be retrieved with a c-style interface")
            {
                void* retrieved_data;
            ...
            {
                void* retrieved_data = malloc(dims[0] * dims[1] * dims[2] * sizeof(float));
        ...
        AND_WHEN("A meta scalar is added to the DataSet")
        {
            ...
            THEN("The meta scalar can be retrieved")
            {
                ...
                dataset.get_meta_scalars(meta_scalar_name, (void*&)retrieved_data,
        ...
        AND_WHEN("A meta string is added to the DataSet")
        {
            ...
            THEN("The meta string can be retrieved")
            {
                ...
                size_t n_strings;
                size_t* lengths;
                dataset.get_meta_strings(meta_str_name, meta_str_data, n_strings, lengths);
                CHECK(*meta_str_data == meta_str_val);
            }
            THEN("The meta strings can be retrieved with only their names")
            {
                std::vector<std::string> meta_str_data = dataset.get_meta_strings(meta_str_name);
