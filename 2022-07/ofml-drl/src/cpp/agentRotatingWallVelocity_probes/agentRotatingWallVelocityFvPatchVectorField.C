...
void Foam::agentRotatingWallVelocityFvPatchVectorField::updateCoeffs()
{
    ...
    if (t >= start_time_)
    {
        ...
        if (steps_remaining && update_omega_)
        {
            ...
            const Foam::functionObject& probeObjFunc = getFuncObj("probes");
...
const Foam::functionObject& Foam::agentRotatingWallVelocityFvPatchVectorField::getFuncObj(Foam::word objName) const
{
    const Foam::functionObjectList& funObjList = this->db().time().functionObjects();
    
    forAll (funObjList, i)
    {
        if (funObjList[i].name() == objName)
        {
            return funObjList[i];
        }
    }

    // TODO: need to raise an exception if function object is not found
}
