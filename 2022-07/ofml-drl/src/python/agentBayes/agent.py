...
class FCPolicy(pt.nn.Module):
    def __init__(self, n_states: int, n_actions: int, action_min: pt.Tensor,
        ...
        #TODO: Layers should be BNN layers
    ...
    @pt.jit.ignore
    #TODO: What are these predictions?
...
class FCPolicyBayesian(pt.nn.Module):
    def __init__(self, n_states: int, n_actions: int, action_min: pt.Tensor,
                 action_max: pt.Tensor, n_layers: int = 2, n_neurons: int = 64,
                 activation: Callable = pt.nn.functional.relu,
                 prior_mu: float = 0., prior_sigma: float = 0.5):
        super(FCPolicyBayesian, self).__init__()
        self._n_states = n_states
        self._n_actions = n_actions
        self._action_min = action_min
        self._action_max = action_max
        ...
        #TODO: Layers should be BNN layers
        self._layers = pt.nn.ModuleList()
        self._layers.append(bnn.BayesLinear(prior_mu=self._prior_mu, prior_sigma=self._prior_sigma, in_features=self._n_states, out_features=self._n_neurons))
        if self._n_layers > 1:
            for hidden in range(self._n_layers - 1):
                self._layers.append(bnn.BayesLinear(prior_mu=self._prior_mu, prior_sigma=self._prior_sigma,
                    in_features=self._n_neurons, out_features=self._n_neurons))
        #self._layers.append(pt.nn.Linear(self._n_neurons, 2*self._n_actions))
        self._last_layer = bnn.BayesLinear(prior_mu=self._prior_mu, prior_sigma=self._prior_sigma, in_features=self._n_neurons, out_features=2*self._n_actions)
